@section('title', trans('admin.viewing_attribute'))
	...
	<div class="row">
		<div class="col-lg-12">
			<div class="page-header">
			  <h1>{{ trans('admin.category_for_tricks') }} <span class="pull-right"> <a data-toggle="modal" href="#add_category" class="btn btn-primary btn-lg">{{ trans('admin.add_new_attribute') }}</a></span></h1>
			</div>
			...
			<table class="table">
			   ...
			   <tbody id="sortable">
			    ...
			    <tr rel="{{ $attribute->id }}">
			        <td><a href="{{url('admin/attribute/view/'.$attribute->id)}}">{{ $attribute->name }}</a></td>
			        <td>{{ $attribute->value}}<br>
			        </td>
			        <td>{{ $attribute->products->count()}}</td>
			        <td>
			        	<div class="btn-group pull-right">
				        <a class="btn btn-primary btn-sm" href="{{url('admin/attribute/view/'.$attribute->id)}}">{{ trans('admin.edit') }}</a>
				        <a class="delete_toggler btn btn-danger btn-sm" rel="{{$attribute->id}}">{{ trans('admin.delete') }}</a>
			        	</div>
			        </td>
			     </tr>
			    @endforeach
     ...
     <div class="modal-content">
       ...
       <div class="modal-body">
            ...
            <div class="form-group">
                ...
                <div class="col-lg-10">
                    {{ Form::textarea('value',null,array('class'=>'form-control','rows'=>'4'))}}
                </div>
            </div>
        	<div class="form-group">
	...
	$(document).ready(function(){
		...
		$('.delete_toggler').each(function(index,elem) {
		    $(elem).click(function(e){
		    	e.preventDefault();
		    	var href = "{{url('admin/attribute/delete')}}/";
