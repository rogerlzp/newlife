...
class YoukuUploader
{
	const ACCESS_CODE_URL = "https://openapi.youku.com/v2/oauth2/authorize";
	...
	private function getCode() {
		$parameter = array(
				...
				"response_type"  => 'code',
				"state"          => 'helo',
				"redirect_uri"          => 'http://www.financehouse.com.cn/youku/token',
		);
		try {
			$result = json_decode(Http::get(self::ACCESS_TOKEN_URL, $parameter));
			echo "22222222222222";
	...
	private function getAccessToken($params) {
		$parameter = array(
			...
			"response_type"  => 'code',
			"state"          => 'helo',
			"redirect_uri"          => 'www.financehouse.com.cn',
	//		"client_secret" => $this->client_secret,
		...
		try {
			$result = json_decode(Http::get(self::ACCESS_CODE_URL, $parameter));
			if (isset($result->error)) {
				echo "111111111111";
				$error = $result->error;
				echo $error->code;
